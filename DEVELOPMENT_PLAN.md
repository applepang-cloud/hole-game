...
> 현재 구현: 테마 맵 6종(방·마당·마트·학교·도시·저승), 구멍 메우기/빠짐(보라=안전·검정=위험), 모든 구멍 제거 시 클리어, 몬스터 추격·공격, 공중 폭격(헬기 30초), 체력(HP) 게이지, 일시정지. 다음 확장: **멀티플레이 경쟁 모드(§11)**.
...
### Phase 5 — 콘텐츠
- [ ] 20월드 테마·1,000레벨 데이터 생성, 밸런싱 패스, 사운드/이펙트 폴리시
